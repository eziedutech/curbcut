...
| Command | Does |
|---|---|
| `uv run --project codes/engine curbcut scan --base main --head <branch> --out reports/curbcut/<id>/facts.json` | Checks out base and head in temporary git worktrees, serves each fixture, and measures every changed page: axe-core (WCAG 2.2 A and AA tags plus best practices, with `target-size` switched on), text contrast by the WCAG formula, a Tab crawl for traps, positive tabindex and missing focus indicators, pointer listeners on elements a keyboard cannot reach, and the accessibility tree snapshot. Each fact says whether head introduced it. Accessibility tree snapshots go to `snapshots.json` beside it. |
