...
export function Layout({ children }: { children: React.ReactNode }) {
  return (
    <html lang="en">
      ...
      <body>
        <Theme appearance="light" accentColor="blue" grayColor="slate" radius="none" panelBackground="solid">
          {children}
        </Theme>
        <ScrollRestoration />
        <Scripts />
...
export default function App() {
  ...
  return <AppShell>{navigation.state === "loading" ? <PageSkeleton /> : <Outlet />}</AppShell>;
}
...
export function ErrorBoundary({ error }: Route.ErrorBoundaryProps) {
  ...
  if (isRouteErrorResponse(error)) {
    ...
    detail = error.status === 404 ? "The requested page or report does not exist." : error.statusText || detail;
